...
export function CategoriesTable({
  ...
  const openEditDialog = (category: z.infer<typeof schema>) => {
    setEditingCategory(category)
    setFormNome(category.nome)
    setFormDescricao(category.descricao)
    setFormStatus(category.status)
    setFormDialogOpen(true)
  }

  const handleSave = () => {
    const action = editingCategory ? "editada" : "criada"
    toast.promise(new Promise((resolve) => setTimeout(resolve, 500)), {
      loading: editingCategory
        ? `Salvando ${formNome}...`
        : `Criando ${formNome}...`,
      success: `Categoria ${action} (simulado)`,
      error: "Erro",
    })
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {categoryToDelete?.nome}"? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                toast.promise(
                  ...
                  {
                    loading: `Excluindo ${categoryToDelete?.nome}...`,
