...
import { ProjectTimeTable } from "~/components/reports/project-time-table"
import { ReportsService, ApiError } from "~/lib/api"
import type { ProjectTimeEntry, ScreenshotFilters } from "~/lib/api/types"

export default function ProjectTimeReportPage() {
  const [data, setData] = useState<ProjectTimeEntry[]>([])
  ...
  useEffect(() => {
    ...
    ReportsService.projectTime(filters)
      .then((res) => {
        if (!cancelled) {
          setData(res.data)
          setIsLoading(false)
          ...
          <div className="flex items-center justify-between px-4 lg:px-6">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-muted-foreground">
                Duração total de trackings agrupados por projeto.
              </p>
            </div>
